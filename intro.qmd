...
See @alam2006 for additional discussion on modeling
...
![](img/param-2.png) - drug can move in both direction k: rate constants A ;amount of drug in the originative compartment

Next the clearance parameters ![](img/param-3.png) Clearance and microcompartment models are usally equivalent and gives similar results. However, clearance models are more commonly used.

![](img/param-4.png) \## 3. administration
...
-   replaces clearance parameter with Km and Vmax ![](img/saturation-1.png) Clearance is no longer constant Clearance is depended on the concengtratino downward bend is characteristics of saturating kinetics saturation increased with higher doses

-   ![](img/saturation-2.png) Let's see how this change affects the shape of the PK curve click next to continue ![](img/saturation-3.png) here are simulated curves for a drug that has a KM value of 1,000 in a saturating model clearance becomes the expression v max over km + c this means that clearance depends on the concentration and the clearance is no longer constant notice how different doses have PK curves with dramatically different shapes notice how many of the curves have a downward bend this downward bend is a characteristic of saturating kinetics it may not be observed at lower doses but as the dose increases we may encounter saturation and it is useful to know what it looks like at the lowest dose shown here the yellow curve looks just like a one compartment model with linear kinetics as the dose increases we see more saturation notice how
